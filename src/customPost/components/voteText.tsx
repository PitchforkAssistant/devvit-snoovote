    ...
    <zstack width="100%" height="100%" grow>
        ...
        {vote.choices.length === 4 &&
            <vstack width="100%" height="100%">
                <hstack width="100%" height="50%">{vote.choices.slice(0, 2).map(choice => <ChoiceText choice={choice} votes={getVotes(choice.id)}/>)}</hstack>
                <hstack width="100%" height="50%">{vote.choices.slice(2, 4).map(choice => <ChoiceText choice={choice} votes={getVotes(choice.id)}/>)}</hstack>
            </vstack>
        }
    </zstack>
);
